...
- 既存ファイル: 施設GID・レビューGIDで既存行と照合し、重複を除いて新規分を追加する。新しいIDは既存の採番の続きになる
- 新規ファイル: 独立したCSVを作り、レビューIDは1、施設IDは101から採番する
- 増分ファイル: 今回新たに追加された行だけを保存する

受入テストでは必ず存在しない新規ファイル名を使います。詳しい動作は[`USER_OPERATION_MANUAL.md`](USER_OPERATION_MANUAL.md)の「出力ファイルの選び方」を参照してください。

## 4. テスト1: Google Places API版

住所CSVは検索地域、検索キーワードは施設の種類を決めます。例えば住所行`北海道,札幌市`と検索キーワード`老人ホーム`は、`北海道 札幌市 老人ホーム`として検索されます。住所CSVの行数を増やすと検索範囲と最低リクエスト数が増えるため、受入テストでは少数行の`address_test.csv`を使用します。テンプレートの詳細は[`ADDRESS_CSV_GUIDE.md`](ADDRESS_CSV_GUIDE.md)を参照してください。
...
歯科医院の過去runは約60時間22分でしたが、134バッチ中94成功・40失敗の部分完了です。成功分66,358件をマージし34,227件を新規追加してPrivateリポジトリへ保存でき、6時間timeoutは0件でした。現在は1並列固定のため、受入テストでは小規模件数だけを確認し、全件時間は10件、100件、1バッチの実測から再見積もりします。

小規模テストでも`partial`になった場合は合格にせず、Issueに表示された失敗バッチ番号、開始行、終了行、完了チャンク数を記録します。成功分のCSVが保存されていることを確認したうえで、同じ出力CSVを選び、`start_from_batch`と必要なら`max_batches`で失敗範囲だけを再実行します。自動再試行は行われません。
...
- 新規レビューCSVはレビューIDが1、新規施設CSVは施設IDが101から始まっている
...
| 新規出力なのにレビューIDが1から始まらない | 同名ファイルがすでに`results/`に存在し、既存出力として扱われていないか確認する |
| 同じGIDの行が重複する | GID列の空欄・手作業変更・同一出力への同時実行を確認する |
| 住所CSVが選択肢に出ない | ヘッダー・UTF-8・住所データを確認し、「WebApp用ファイル一覧・住所CSV検証」を実行する |
| `住所CSVのヘッダーが不正です` | 1行目を`都道府県,市区町村,町域`形式にする |
| `住所CSVの2行目の都道府県が不正です` | 1列目を正式な都道府県名へ修正する |
