...
施設検索では、住所CSVの各行と検索キーワードを結合します。例えば`北海道,札幌市`と`歯科医院`を選ぶと、`北海道 札幌市 歯科医院`として検索します。住所CSVの作成方法、検索範囲と費用の関係、不正テンプレートのエラー例は[`ADDRESS_CSV_GUIDE.md`](ADDRESS_CSV_GUIDE.md)を参照してください。
